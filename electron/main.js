...
// 캐시 설정 - 메모리 및 디스크 최적화
const CACHE_CONFIG = {
  MAX_SIZE: 500,                     // 최대 캐시 항목 수 (1000 → 500으로 감소)
  MAX_MEMORY_MB: 30,                 // 최대 메모리 사용량 (MB) (50 → 30으로 감소)
  MAX_DISK_MB: 100,                  // 최대 디스크 캐시 크기 (MB) - 새로 추가
  DEFAULT_TTL: 3 * 24 * 60 * 60 * 1000, // 기본 TTL: 3일 (7일 → 3일로 감소)
  CLEANUP_INTERVAL: 30 * 60 * 1000,  // 정리 주기: 30분 (1시간 → 30분으로 단축)
  SAVE_INTERVAL: 10 * 60 * 1000,     // 저장 주기: 10분 (5분 → 10분으로 변경, 디스크 I/O 감소)
};
...
function checkDiskCacheSize() {
  try {
    ...
    // 디스크 캐시 크기가 최대치를 넘으면 정리
    if (fileSizeMB > CACHE_CONFIG.MAX_DISK_MB) {
      console.warn(`⚠️ [Electron Main] 디스크 캐시 크기 초과: ${fileSizeMB.toFixed(2)}MB > ${CACHE_CONFIG.MAX_DISK_MB}MB`);
      
      // 가장 오래된 캐시 항목부터 30% 제거
      const items = Array.from(nativeCache.entries())
        .map(([key, item]) => ({
          key,
      ...
      let removedSize = 0;
      const targetSize = CACHE_CONFIG.MAX_DISK_MB * 0.7 * 1024 * 1024; // 70%까지 감소
      ...
      console.log(`🧹 [Electron Main] 디스크 캐시 정리 완료: ${(removedSize / 1024 / 1024).toFixed(2)}MB 제거`);
    }
  } catch (error) {
    console.warn('⚠️ [Electron Main] 디스크 캐시 크기 확인 실패:', error.message);
  }
}
...
function checkMemoryUsage() {
  try {
    ...
    if (heapUsedMB > CACHE_CONFIG.MAX_MEMORY_MB) {
      ...
      // 메모리 사용량에 따라 정리 비율 조정
      const usageRatio = heapUsedMB / CACHE_CONFIG.MAX_MEMORY_MB;
      let evictRatio = 0.2; // 기본 20%
      
      if (usageRatio > 1.5) {
        evictRatio = 0.5; // 150% 초과 시 50% 제거
      } else if (usageRatio > 1.2) {
        evictRatio = 0.3; // 120% 초과 시 30% 제거
      }
      
      const evictCount = Math.ceil(nativeCache.size * evictRatio);
      evictLRUItems(evictCount);
      console.log(`🧹 [Electron Main] 메모리 압박으로 캐시 ${evictCount}개 정리 (${(evictRatio * 100).toFixed(0)}%)`);
...
// 앱 시작 시 캐시 로드 (스플래시 표시 전에 실행)
// loadNativeCache()는 app.on('ready') 이전에 실행되므로 스플래시 화면이 표시된 후에 상태 업데이트
...
// Firebase Storage는 Generic 서버로 사용 (Discord처럼 백그라운드 자동 업데이트)
...
// 프로덕션 환경에서만 업데이트 서버 설정
if (app.isPackaged && !preferDevServer) {
  // electron-updater에 Firebase Storage를 Generic 서버로 설정
  // latest.yml 파일의 공개 URL (Firebase Storage 공개 접근)
  ...
  console.log('[Updater] latest.yml URL:', latestYmlUrl);
}

// autoUpdater 설정 - Discord처럼 자동으로 백그라운드 업데이트
autoUpdater.autoDownload = true; // ✅ 자동 다운로드 활성화 (사용자 승인 없이)
...
function createSplashWindow() {
  const splashPath = getResourcePath('electron/splash.html');
  ...
  splashWindow.loadFile(splashPath).then(() => {
    splashWindow.show();
    console.log('✅ [Electron Main] 스플래시 화면 표시');
    
    // 스플래시 화면에서 이미지 경로 업데이트
    ...
    console.warn('⚠️ [Electron Main] 스플래시 화면 로드 실패:', error.message);
    // 스플래시 화면 로드 실패 시 메인 윈도우 바로 표시
    if (mainWindow) {
      mainWindow.show();
    }
  });
  ...
  if (splashWindow && !splashWindow.isDestroyed()) {
    ...
    setTimeout(() => {
      if (splashWindow && !splashWindow.isDestroyed()) {
        splashWindow.close();
        splashWindow = null;
        console.log('✅ [Electron Main] 스플래시 화면 닫기 완료');
      }
    }, 150);
  }
}
...
/**
 * 스플래시 화면 상태 업데이트
 * @param {string} message - 메인 상태 메시지
 * @param {string} detail - 상세 설명 (선택적)
 * @param {number} progress - 진행률 (0-100, 선택적)
 */
...
function createWindow() {
  mainWindow = new BrowserWindow({
    ...
    icon: (() => {
      ...
      // 기본값
      return getResourcePath('public/tms-logo.png');
    })(),
  ...
  const load = async () => {
    updateSplashStatus('세션 캐시 정리 중...', '이전 세션 데이터 초기화', 25);
    // 세션 캐시 정리는 백그라운드에서 진행 (블로킹 안 함)
    mainWindow.webContents.session.clearCache().catch(() => {});
    
    if (app.isPackaged) {
      // 프로덕션 빌드: 파일 시스템에서 직접 로드 (HTTP 서버 없이, 네이티브처럼 빠름)
      updateSplashStatus('애플리케이션 파일 로드 중...', '로컬 파일 시스템에서 읽기', 30);
      const indexPath = getResourcePath('dist/index.html');
      console.log(`✅ [Electron Main] 파일 직접 로드: ${indexPath}`);
      await mainWindow.loadFile(indexPath);
    } else {
      // 개발 모드: 개발 서버 사용
      updateSplashStatus('개발 서버 연결 중...', 'Vite 개발 서버에 연결', 30);
      devServerInUse = true;
      console.log(`✅ [Electron Main] 개발 서버 사용: ${DEV_SERVER_URL}`);
      await mainWindow.loadURL(DEV_SERVER_URL);
    }
    
    // ✅ HTML 로드 완료 후 즉시 메인 윈도우 표시 (로컬 프로그램처럼 빠르게)
    updateSplashStatus('UI 준비 완료', '애플리케이션 인터페이스 로드됨', 50);
    mainWindow.show();
    ...
    // ✅ Firebase 초기화와 프리로딩은 백그라운드에서 진행 (UI 블로킹 안 함)
    // Firebase와 데이터는 렌더러 프로세스에서 비동기로 초기화됨
    // UI는 이미 표시되어 있고, 데이터는 로드되면서 채워짐
    
    console.log('✅ [Electron Main] 메인 윈도우 표시 완료 (Firebase 초기화는 백그라운드에서 진행)');
  };
  load();
...
function createTray() {
  try {
    // Windows에서는 .ico 파일을 우선 사용 (더 안정적)
    // 여러 크기의 아이콘을 시도 (16x16, 32x32 등)
    let iconPath = null;
    let trayIcon = null;
    
    // 아이콘 파일 경로 우선순위
    const iconPaths = [
      'public/tms-logo.ico',  // Windows용 .ico 파일 (우선)
      'public/icon.ico',       // 대체 .ico 파일
      'public/tms-logo.png',   // PNG 파일 (폴백)
    ];
    ...
    for (const relativePath of iconPaths) {
      ...
      if (fs.existsSync(fullPath)) {
        iconPath = fullPath;
        console.log(`✅ [Electron Main] 시스템 트레이 아이콘 사용: ${fullPath}`);
        break;
      }
    }
    ...
    // 아이콘 파일을 찾지 못한 경우
    if (!iconPath) {
      console.error('❌ [Electron Main] 시스템 트레이 아이콘 파일을 찾을 수 없습니다.');
      console.error('   시도한 경로:', iconPaths.map(p => getResourcePath(p)));
      // 빈 아이콘으로 트레이 생성 (최소한 트레이는 표시됨)
      trayIcon = nativeImage.createEmpty();
    } else {
      // 아이콘 로드
      trayIcon = nativeImage.createFromPath(iconPath);
      
      // Windows 시스템 트레이에서는 16x16 또는 32x32 권장
      // .ico 파일은 여러 크기를 포함할 수 있으므로 resize가 필요할 수 있음
      if (process.platform === 'win32') {
        // Windows에서는 16x16 또는 32x32 크기로 리사이즈
        const systemIconSize = 16; // Windows 시스템 트레이 기본 크기
        if (trayIcon.getSize().width !== systemIconSize) {
    ...
    console.log('✅ [Electron Main] 시스템 트레이 생성 완료');
  } catch (error) {
    console.error('❌ [Electron Main] 시스템 트레이 생성 실패:', error);
...
function setupUpdater() {
  ...
  // 업데이트 다운로드 완료 - Discord처럼 백그라운드 다운로드만 (설치는 앱 종료 시)
  autoUpdater.on('update-downloaded', (info) => {
    console.log('[Updater] 다운로드 완료:', info.version);
    console.log('[Updater] 앱을 종료하면 자동으로 설치되고 재시작됩니다.');
    ...
    // ⚠️ 앱 사용 중에는 설치하지 않음 (작업 방해 방지)
    // 설치와 재시작은 앱 종료 시 자동으로 수행됨 (autoInstallOnAppQuit = true)
  });
...
if (!gotTheLock) {
  ...
  app.on('ready', () => {
    ...
    setTimeout(async () => {
      ...
      updateSplashStatus('네이티브 캐시 로드 중...', '로컬 캐시 데이터 활성화', 5);
      await new Promise(resolve => setTimeout(resolve, 100));
      
      updateSplashStatus('초기 설정을 확인하는 중...', '시스템 설정 로드', 8);
      await new Promise(resolve => setTimeout(resolve, 50));
      
      // 성능 최적화: 앱 우선순위 설정 (Windows)
      if (process.platform === 'win32') {
        try {
          updateSplashStatus('프로세스 우선순위 설정 중...', '성능 최적화 적용', 10);
      ...
      // IPC 핸들러 등록 (윈도우 컨트롤용)
      updateSplashStatus('IPC 핸들러 등록 중...', '통신 시스템 초기화', 12);
      registerIpcHandlers();
      await new Promise(resolve => setTimeout(resolve, 50));
      
      // 전역 에러 핸들러 등록
      updateSplashStatus('에러 핸들러 설정 중...', '오류 처리 시스템 준비', 15);
      setupGlobalErrorHandlers();
      await new Promise(resolve => setTimeout(resolve, 50));
      
      // 업데이터 설정
      updateSplashStatus('업데이트 시스템 설정 중...', '자동 업데이트 준비', 18);
      setupUpdater();
      await new Promise(resolve => setTimeout(resolve, 50));
      
      updateSplashStatus('메인 윈도우 생성 중...', '애플리케이션 인터페이스 준비', 20);
...
function startPerformanceMonitoring() {
  ...
  setInterval(() => {
    try {
      ...
      // 메모리 사용량이 높으면 경고 및 자동 정리 (200MB → 150MB로 낮춤)
      if (heapUsedMB > 150) {
