...
import { ProductionTrendData, ProductionHistoryItem, CoatingHistoryItem, QualityInspectionItem, QualityIssueItem } from '@/features/production/types/product.types';
...
export const analyzeProductionTrend = async (
  ...
  try {
    ...
    // gemini-2.5-flash 또는 gemini-3.0-flash 우선 사용
    // 모델이 없으면 자동으로 fallback됨
    let model;
    try {
      // gemini-2.5-flash 우선 시도
