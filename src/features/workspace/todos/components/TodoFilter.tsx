...
import type { TodoFilter as TodoFilterType, TodoSortBy, TodoFilterOptions } from '../types/todo.types';
  ...
  return (
    <div className={cn('flex items-center gap-2 px-4 py-2.5 border-b bg-muted/10', className)}>
      ...
      <div className="flex items-center gap-0.5 border-r border-border pr-2 mr-1">
        {(['all', 'my-todos', 'incomplete', 'completed', 'overdue'] as TodoFilterType[]).map((filter) => (
          <Button
            key={filter}
            variant="ghost"
            size="sm"
