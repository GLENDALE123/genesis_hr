...
export class ChannelService {
  ...
  static async createFolder(
    ...
    const now = new Date().toISOString();
    const folderData: Omit<Channel, 'id'> = {
      workspaceId,
      name,
      type: 'public',
      members: [createdBy],
    ...
    const cleanedData = removeUndefinedFields(folderData);
    const channelsRef = collection(db, getChannelsCollectionPath(workspaceId));
    const docRef = await addDoc(channelsRef, cleanedData);
    return docRef.id;
  }
   ...
   * 채널을 폴더로 이동
   */
  static async moveChannelToFolder(
    channelId: string,
    folderId: string,
    workspaceId: string
  ): Promise<void> {
    ...
    // 폴더 업데이트: folderChannelIds에 채널 ID 추가
    const folderRef = doc(db, getChannelsCollectionPath(workspaceId), folderId);
    const folderDoc = await getDoc(folderRef);
    if (folderDoc.exists()) {
      const folderData = folderDoc.data() as Channel;
      const currentChannelIds = folderData.folderChannelIds || [];
      if (!currentChannelIds.includes(channelId)) {
        batch.update(folderRef, {
          folderChannelIds: arrayUnion(channelId),
          updatedAt: new Date().toISOString(),
        });
      }
    }
  ...
  static async updateChannelOrder(
    workspaceId: string,
    channelOrders: Array<{ id: string; order: number }>
  ): Promise<void> {
    if (!db) throw new Error('Firestore is not initialized');
  ...
  static async deleteFolder(
    ...
    const folderData = folderDoc.data() as Channel;
    const folderChannelIds = folderData.folderChannelIds || [];

    // 폴더 안의 채널들을 폴더 밖으로 이동
    folderChannelIds.forEach((channelId) => {
