    ...
    const fetchLatestRelease = async () => {
      try {
        ...
        try {
          ...
          const latestInfo: ElectronRelease = await response.json();

          // 실제 설치 파일의 다운로드 URL 가져오기
          // fileName이 있으면 사용, 없으면 고정 파일명 사용 (하위 호환성)
          const fileName = latestInfo.fileName || 'TMS-Setup-latest.exe';
          const installerRef = ref(storage, `electron-releases/${fileName}`);
  ...
  return {
    latestRelease: latestRelease
      ? {
          ...
          name: `TMS 통합관리시스템 v${latestRelease.version}`,
