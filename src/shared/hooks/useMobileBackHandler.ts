...
export const useMobileBackHandler = ({
  ...
  useEffect(() => {
    ...
    const handlePopState = (_event: PopStateEvent) => {
      ...
      if (historyStack.length > 0) {
        ...
        // 가장 최근에 열린 컴포넌트만 닫기
        if (topItem.componentId === componentId.current) {
          // 최신 onClose 함수 사용
          topItem.onClose();
    ...
    if (!historyStateAdded) {
      ...
      // 스택에 추가 (가장 최근 것) - 최신 onClose 함수 참조 사용
      historyStack.push({ componentId: componentId.current, onClose: () => onCloseRef.current() });
    } else {
  ...
  }, [isOpen, historyStateAdded, isMobile, componentType]);
