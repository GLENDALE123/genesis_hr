...
export const ChatRoomPageClient: React.FC<ChatRoomPageClientProps> = ({
  ...
  // 채팅 메시지 전송 핸들러
  const handleSendMessage = async (text: string, mentionedUserIds?: string[]) => {
    if (!user || !currentChatRoom) return;

    // 이름+직급 가져오기 (UserList에서 사용하는 방식과 동일)
    const senderDisplayName = getUserDisplayName(
      { displayName: user.displayName || userProfile?.displayName, email: user.email },
    ...
    try {
      ...
      if (currentChatRoom.id.startsWith('temp_') && result.roomId !== chatRoomId) {
  ...
  return (
    <div className="flex h-full min-w-0 w-full max-w-full">
      {!isMobile && <ChatSidebar className="flex-shrink-0" />}
